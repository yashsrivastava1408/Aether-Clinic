...
def verify_response(
    ...
    for trigger, rule in MANDATORY_WARNINGS.items():
        ...
        if trigger in query_lower or trigger in context_lower:
            ...
            has_safety_content = any(
                ...
                if check_word in response_lower
            )
    ...
    if warnings_added:
        safety_additions += "\n\n" + "━" * 40 + "\n"
        safety_additions += "🛡️ SAFETY OVERSIGHT SYSTEM\n"
        safety_additions += "━" * 40 + "\n"
        safety_additions += "\n".join(warnings_added)
    
    # 2. ALWAYS add the legal disclaimer footer
    safety_additions += MANDATORY_FOOTER
    
    modified_response = ai_response + safety_additions
