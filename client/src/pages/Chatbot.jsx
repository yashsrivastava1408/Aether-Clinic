...
export default function Chatbot({ doctor, onBack }) {
  ...
  const sanitizeMessageText = (text = "") => {
    ...
    for (const line of lines) {
      ...
      if (
        trimmed === "🛡️ SAFETY OVERSIGHT SYSTEM" ||
        trimmed === "⚖️ MEDICAL LEGAL DISCLAIMER" ||
        /^━+$/.test(trimmed) ||
        /^-{3,}$/.test(trimmed) ||
        ...
        trimmed.startsWith("SECURE LINK ESTABLISHED") ||
        trimmed.startsWith("NEURAL LINK ACTIVE") ||
        trimmed === "[ EXECUTE ]" ||
        trimmed === "ACCURATE?" ||
        trimmed.startsWith("This response is generated by an AI assistant") ||
        trimmed.startsWith("It is NOT a substitute for professional medical advice") ||
        trimmed.startsWith("Always seek the advice of your physician") ||
        trimmed.startsWith("Always consult a physician") ||
        trimmed.startsWith("Never disregard professional medical advice") ||
        trimmed.startsWith("IF YOU ARE EXPERIENCING A MEDICAL EMERGENCY")
      ) {
        stop = true;
      }
      if (!stop) cleaned.push(line);
    }
  ...
  return (
    <div className={`md:h-[85vh] h-[80vh] w-full max-w-5xl mx-auto flex flex-col relative overflow-hidden rounded-3xl border shadow-2xl transition-colors duration-500 ${isDark ? 'bg-[#030303] border-white/10' : 'bg-white border-slate-200'}`}>
      ...
      <div className="absolute inset-0 opacity-[0.05] pointer-events-none transition-all duration-1000"
        style={{
          backgroundImage: `linear-gradient(${isPremium ? 'rgba(245, 158, 11, 0.2)' : 'rgba(16, 185, 129, 0.1)'} 1px, transparent 1px),
                                  linear-gradient(90deg, ${isPremium ? 'rgba(245, 158, 11, 0.2)' : 'rgba(16, 185, 129, 0.1)'} 1px, transparent 1px)`,
          backgroundSize: '30px 30px'
        }}
      />
      <div className={`absolute inset-0 bg-gradient-to-b pointer-events-none transition-colors duration-1000 ${isPremium ? (isDark ? 'from-cyan-900/10 via-transparent to-cyan-900/20' : 'from-cyan-500/5 via-transparent to-cyan-500/10') : (isDark ? 'from-emerald-900/10 via-transparent to-emerald-900/20' : 'from-emerald-500/5 via-transparent to-emerald-500/10')}`} />
      ...
      <div className={`relative z-10 p-6 border-b flex items-center justify-between backdrop-blur-md ${isDark ? 'border-white/5 bg-[#0a0a0a]/80' : 'border-slate-100 bg-white/80'}`}>
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className={`flex items-center gap-2 text-[10px] font-mono uppercase tracking-widest ${isPremium ? 'text-cyan-500' : 'text-emerald-500'}`}>
              <span className={`w-1.5 h-1.5 rounded-full animate-pulse ${isPremium ? 'bg-cyan-500' : 'bg-emerald-500'}`} />
              {isPremium ? 'Secure Link • Aether+ Pro' : 'Secure Link Established'}
            </div>
          </div>
        </div>
      ...
      {/* System Boot Overlay */}
      {isInitializing && (
        <div className={`absolute inset-0 z-50 flex items-center justify-center font-mono text-xs transition-colors duration-1000 ${isDark ? 'bg-[#030303]' : 'bg-slate-50'} ${isPremium ? 'text-cyan-500' : 'text-emerald-600'}`}>
          <div className="space-y-2">
            <div className={`animate-typewriter overflow-hidden whitespace-nowrap border-r pr-1 ${isPremium ? 'border-cyan-500' : 'border-emerald-500'}`}>
        ...
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.sender === "user" ? "justify-end" : "justify-start"}`}>
            ...
            {msg.sender === 'ai' && !msg.isSystem && (
              <div className={`w-8 h-8 rounded-full border flex items-center justify-center mr-3 mt-1 text-xs transition-all duration-500 ${isPremium ? (isDark ? 'bg-cyan-900/40 border-cyan-500/50 shadow-[0_0_15px_rgba(6,182,212,0.2)]' : 'bg-cyan-100 border-cyan-300 shadow-sm') : (isDark ? 'bg-emerald-900/30 border-emerald-500/30' : 'bg-emerald-100 border-emerald-200')}`}>
                🤖
              </div>
            )}

            <div className={`max-w-[75%] rounded-2xl p-4 text-sm leading-relaxed backdrop-blur-sm border whitespace-pre-wrap transition-colors duration-500 ${msg.isSystem
              ? `w-full text-center bg-transparent border-transparent font-mono text-xs my-2 ${isPremium ? 'text-cyan-500/50' : 'text-emerald-500/50'}`
              : msg.sender === "user"
                ? "bg-blue-600/20 border-blue-500/30 text-blue-500 font-medium rounded-br-none"
                : (isPremium
                    ? (isDark ? "bg-cyan-500/10 border-cyan-500/30 text-cyan-50 shadow-[0_0_15px_rgba(6,182,212,0.1)] rounded-bl-none" : "bg-cyan-50 border-cyan-200 text-slate-800 shadow-sm rounded-bl-none")
                    : (isDark ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-100 rounded-bl-none" : "bg-emerald-50 border-emerald-200 text-slate-800 shadow-sm rounded-bl-none"))
              }`}>
              {msg.image && (
              ...
              <span className={msg.sender === "user" ? (isDark ? "text-blue-100" : "text-blue-900") : ""}>
                ...
                {(() => {
                  const rawText = sanitizeMessageText(msg.text);
                  // Sanitize for XSS (Defense in Depth)
                  const safeText = msg.sender === 'ai' ? DOMPurify.sanitize(rawText) : rawText;
                  
                  const riskMatch = safeText.match(/\[RISK_ANALYSIS:\s*({.*?})\]/);
                  if (riskMatch) {
                    try {
                      const data = JSON.parse(riskMatch[1]);
                      const cleanText = safeText.replace(/\[RISK_ANALYSIS:.*?\]/, '').trim();
                      return (
              ...
              {msg.sender === "ai" && !msg.isSystem && Array.isArray(msg.citations) && msg.citations.length > 0 && (
                <div className={`mt-4 pt-3 border-t transition-colors duration-500 ${isPremium ? (isDark ? 'border-cyan-500/20' : 'border-cyan-200') : (isDark ? 'border-emerald-500/10' : 'border-emerald-200/80')}`}>
                  <div className="flex items-center gap-2 mb-2">
                    <span className={`text-[10px] font-mono uppercase tracking-widest transition-colors duration-500 ${isPremium ? 'text-cyan-500' : 'text-emerald-500'}`}>
                      Verified Sources
                    </span>
                    <span className={`text-[9px] px-2 py-0.5 rounded-full border transition-colors duration-500 ${isPremium ? (isDark ? 'border-cyan-500/30 text-cyan-300 bg-cyan-500/10' : 'border-cyan-300 text-cyan-700 bg-cyan-50') : (isDark ? 'border-emerald-500/20 text-emerald-300 bg-emerald-500/10' : 'border-emerald-200 text-emerald-700 bg-emerald-50')}`}>
                  ...
                  <div className="space-y-2">
                    {msg.citations.map((citation, idx) => (
                      <div
                        key={`${citation.title || "citation"}-${idx}`}
                      ...
                      >
                        ...
                        <div className="mt-1 flex flex-wrap gap-2 text-[10px] uppercase tracking-wide opacity-70">
                          {citation.category && <span>Category: {citation.category}</span>}
                          {typeof citation.relevance === "number" && <span>Relevance: {Math.round(citation.relevance * 100)}%</span>}
                        </div>
                      </div>
                    ))}
                  </div>
      ...
      <div className={`p-4 border-t relative z-20 ${isDark ? 'bg-[#0a0a0a]/90 border-white/5' : 'bg-slate-50/90 border-slate-200'}`}>
        ...
        {imagePreview && (
          <div className="mb-4 relative inline-block">
            <img src={imagePreview} alt="Preview" className={`h-20 w-20 object-cover rounded-lg border transition-colors duration-500 ${isPremium ? 'border-cyan-500/50' : 'border-emerald-500/50'}`} />
            <button
              onClick={removeImage}
        ...
        <div className={`flex items-center gap-4 p-4 rounded-xl border transition-all duration-500 ${isTyping ? (isPremium ? 'border-cyan-500/50 shadow-[0_0_15px_rgba(6,182,212,0.2)]' : 'border-emerald-500/50 shadow-[0_0_15px_rgba(16,185,129,0.1)]') : (isDark ? 'border-white/10 bg-black/50' : 'border-slate-300 bg-white shadow-inner')} `}>
          ...
          <span className={`font-mono text-lg transition-colors duration-500 ${isPremium ? 'text-cyan-500' : 'text-emerald-500'}`}>{'>'}</span>
          <input
            value={input}
          ...
          <button
            ...
            className={`text-xs font-mono disabled:text-gray-400 uppercase tracking-wider transition-colors duration-500 ${isPremium ? 'text-cyan-500 hover:text-cyan-400' : 'text-emerald-500 hover:text-emerald-600'}`}
          >
            [ EXECUTE ]
          </button>
        </div>
        <div className="flex justify-between mt-2 px-2 text-[10px] text-gray-500 font-mono uppercase">
          <span>TOKENS: {tokenUsage.toLocaleString()} / {user?.isGuest ? '10,000' : '100,000'} {isPremium && <span className="text-cyan-500 ml-2">⚡️ GROQ ENABLED</span>}</span>
          ...
          {messages.length > 2 && !messages[messages.length - 1]?.text?.includes("FINAL MEDICAL ASSESSMENT") && (
            <button
              ...
              className="text-cyan-500 hover:text-cyan-400 transition-colors cursor-pointer flex items-center gap-1"
            >
