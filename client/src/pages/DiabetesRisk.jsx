...
export default function DiabetesRisk() {
    ...
    const labels = [
        { label: "Pregnancies", placeholder: "Number of pregnancies", icon: "🤰" },
        { label: "Blood Sugar", placeholder: "Glucose level (mg/dL)", icon: "🩸" },
        { label: "Blood Pressure", placeholder: "Resting BP (mm Hg)", icon: "🩺" },
        { label: "Skin Thickness", placeholder: "Skin fold thickness (mm)", icon: "📏" },
        { label: "Insulin Level", placeholder: "Serum insulin (mu U/ml)", icon: "💉" },
        { label: "Body Mass (BMI)", placeholder: "Weight-to-height ratio", icon: "⚖️" },
        { label: "Family History", placeholder: "Diabetes pedigree Score", icon: "🧬" },
        { label: "Patient Age", placeholder: "Age in years", icon: "👤" }
    ];
    ...
    const submit = async () => {
        ...
        setResult(null);
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5050'}/api/ml/diabetes`, {
                method: "POST",
                ...
                <div className="mb-12 text-center relative">
                    <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full border mb-4 animate-fadeIn ${isDark ? 'bg-emerald-900/20 border-emerald-500/20' : 'bg-white border-emerald-200 shadow-sm'}`}>
                        ...
                        <span className={`text-[10px] font-mono tracking-widest uppercase ${isDark ? 'text-emerald-400' : 'text-emerald-600'}`}>
                            System Active // Diabetes Diagnostic Module
                        </span>
                    </div>
                    ...
                    <p className={`${isDark ? 'text-gray-400' : 'text-slate-600'} max-w-2xl mx-auto text-lg animate-slideInRight delay-100`}>
                        Enter your health metrics below for a personalized AI-powered assessment of your diabetic risk factors.
                    </p>
                </div>
                            ...
                            <div className={`h-full backdrop-blur-xl border rounded-2xl p-6 shadow-2xl relative overflow-hidden transition-all duration-500 animate-slideInRight ${isDark ? 'bg-white/5 border-white/10' : 'bg-white border-slate-200'}`}>
                                ...
                                <div className="space-y-6 relative z-10">
                                    ...
                                    <div className={`p-4 rounded-lg border text-sm leading-relaxed ${isDark ? 'bg-blue-500/10 border-blue-500/20 text-blue-200/80' : 'bg-blue-50 border-blue-100 text-blue-800'}`}>
                                        ...
                                        {result.prediction
                                            ? "The model suggests elevated diabetic risk. We strongly recommend consulting an endocrinologist for a comprehensive diagnostic screening including HbA1c and FPG tests."
                                            : "Metabolic patterns appear to be within healthy thresholds. Continue maintaining a balanced diet and regular physical activity."}
                                    </div>
                                </div>
                            </div>
                        ) : (
